...
O bot não envia nenhuma transação on-chain no estado atual: ele só simula, registra em log e imprime as oportunidades detectadas.
...
1. Inicializa o cliente HTTP (`reqwest::Client`) aceitando certificados inválidos (`danger_accept_invalid_certs(true)`).
2. Tenta carregar a keypair de `meu-bot.json`; se falhar, gera uma carteira fake para exibir o `pubkey`.
3. Entra em um loop infinito (`loop { ... }`) com as etapas:
...
4. Entre cada iteração completa, o bot dorme 2 segundos; após uma oportunidade real, dorme 10 segundos para não floodar o log.
